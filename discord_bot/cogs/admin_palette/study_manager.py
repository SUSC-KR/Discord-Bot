...
class StudyManager(commands.Cog):
    ...
    async def delete_study(
        self, ctx, year: str, study_name: str, season: str, category_name: str
    ):
        guild = ctx.guild
        season = season.upper()
        ...
        if not category:
            return await ctx.send(
                f"'{category_name}' 카테고리가 존재하지 않습니다."
            )
    ...
    async def create_study_channels(
        self, guild, category, study_name, role_name
    ):
        overwrites = {
        ...
        await guild.create_text_channel(
            f"{study_name}-채팅", category=category, overwrites=overwrites
        )
        await guild.create_voice_channel(
            f"{study_name}-음성", category=category, overwrites=overwrites
        )

    async def delete_study_channels_with_study_name(self, category, study_name):
        for channel in category.channels:
            if (
                f"{study_name.lower()}-채팅" in channel.name
                or f"{study_name.upper()}-음성" in channel.name
            ):
