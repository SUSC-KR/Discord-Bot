...
class StudyManager(commands.Cog):
    ...
    async def delete_study(self, ctx, year: str, study_name: str, season: str, category_name: str):
        ...
        await ctx.send(f"'{category_name}' 카테고리와 관련된 스터디 룸과 역할이 삭제되고, 멤버가 '{season_role_name}' 역할로 이동되었습니다.")
    ...
    async def migrate_members_from_role(self, guild, category, role_name, new_role):
